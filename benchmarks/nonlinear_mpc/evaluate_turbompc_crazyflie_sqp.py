#!/usr/bin/env python3
"""Closed-loop Crazyflie SQP-RTI rollout using TurboMPC as the baseline solver."""
...
from benchmarks.nonlinear_mpc.turbompc_adapter import (
    DenseStageTurbompcProblem,
...
def run(args: argparse.Namespace) -> dict[str, object]:
    ...
    print("building Crazyflie TurboMPC dense-stage problem...", flush=True)
    problem_start = time.perf_counter()
    sparse_problem = make_crazyflie_sqp_problem(args.horizon_steps)
    problem = DenseStageTurbompcProblem(
        sparse_problem,
        state_dim=CRAZYFLIE_NX,
        control_dim=CRAZYFLIE_NU,
        name="CrazyflieDenseStageTurboMPC",
    )
...
def main() -> None:
    ...
    parser.add_argument("--plot-path", type=pathlib.Path, default=pathlib.Path("results/nonlinear_mpc/turbompc_crazyflie_states.png"))
    parser.add_argument("--position-plot-path", type=pathlib.Path, default=pathlib.Path("results/nonlinear_mpc/turbompc_crazyflie_positions.png"))
