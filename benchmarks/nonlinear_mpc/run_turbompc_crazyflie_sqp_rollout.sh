...
CRAZYFLIE_OUTPUT_NAME="${CRAZYFLIE_OUTPUT_NAME:-crazyflie_sqp_rollout_turbompc}"
...
TURBOMPC_CRAZYFLIE_BATCH_SIZES="${TURBOMPC_CRAZYFLIE_BATCH_SIZES:-${CRAZYFLIE_BATCH_SIZES}}"
...
run_benchmark "${CRAZYFLIE_OUTPUT_NAME}" \
  "${PYTHON}" benchmarks/nonlinear_mpc/benchmark_turbompc_crazyflie_sqp_rollout.py \
    --batch-sizes "${TURBOMPC_CRAZYFLIE_BATCH_SIZES}" \
    --dtypes "${DTYPES}" \
    --repeat "${REPEAT}" \
