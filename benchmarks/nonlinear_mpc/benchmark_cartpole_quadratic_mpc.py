#!/usr/bin/env python3
"""Batch-size sweep for the tuned quadratic cartpole closed-loop SQP MPC."""
...
EVALUATOR = ROOT / "benchmarks" / "nonlinear_mpc" / "evaluate_cartpole_quadratic_mpc.py"
...
def _plot_summary(path: pathlib.Path, rows: list[dict[str, object]]) -> None:
    ...
    for idx, (dtype, mode) in enumerate(keys):
        ...
        batch_sizes = np.asarray([int(row["batch_size"]) for row in group])
        ...
        axes[1].plot(
            batch_sizes,
            [float(row["closed_loop_steps_per_s"]) for row in group],
            marker=marker,
            label=label,
        )
        axes[2].plot(
            batch_sizes,
            [float(row["return_mean"]) for row in group],
            marker=marker,
            label=label,
        )
    axes[0].set_ylabel("SQP iterations / s")
    axes[1].set_ylabel("closed-loop steps / s")
    axes[2].set_ylabel("mean rollout return")
    for ax in axes:
...
def run_batch(
    ...
    *,
    ...
    prefix = f"cartpole_quadratic_mpc_{qp_solver}_{dtype}_{mode_slug}_batch_{batch_size}"
    ...
    if proc.returncode == 0 and summary_json.exists():
        ...
        print(
            ...
            f"success={float(summary['rollout_success_rate']):.2%}"
        )
    else:
        row["status"] = "failed"
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=pathlib.Path,
        default=pathlib.Path("results/nonlinear_mpc/cartpole_quadratic_mpc"),
    )
    parser.add_argument(
        "--csv-path",
        type=pathlib.Path,
        default=pathlib.Path("results/nonlinear_mpc/cartpole_quadratic_mpc.csv"),
    )
    parser.add_argument(
        "--plot-path",
        type=pathlib.Path,
        default=pathlib.Path("results/nonlinear_mpc/cartpole_quadratic_mpc_summary.png"),
    )
