#!/usr/bin/env python3
"""Batch-size sweep for the TurboMPC-autodiff humanoid MPC baseline."""
...
EVALUATOR = ROOT / "benchmarks" / "nonlinear_mpc" / "evaluate_turbompc_autodiff_humanoid_mpc.py"
...
def run_batch(args: argparse.Namespace, *, batch_size: int, dtype: str) -> dict[str, object]:
    prefix = f"humanoid_mpc_turbompc_autodiff_{dtype}_batch_{batch_size}"
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=pathlib.Path, default=pathlib.Path("results/nonlinear_mpc/humanoid_mpc_turbompc_autodiff"))
    parser.add_argument("--csv-path", type=pathlib.Path, default=pathlib.Path("results/nonlinear_mpc/humanoid_mpc_turbompc_autodiff.csv"))
    parser.add_argument("--plot-path", type=pathlib.Path, default=pathlib.Path("results/nonlinear_mpc/humanoid_mpc_turbompc_autodiff_summary.png"))
