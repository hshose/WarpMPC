...
HUMANOID_OUTPUT_NAME="${HUMANOID_OUTPUT_NAME:-humanoid_mpc_turbompc_autodiff}"
...
TURBOMPC_AUTODIFF_HUMANOID_BATCH_SIZES="${TURBOMPC_AUTODIFF_HUMANOID_BATCH_SIZES:-${TURBOMPC_HUMANOID_BATCH_SIZES:-${HUMANOID_BATCH_SIZES}}}"

run_benchmark "${HUMANOID_OUTPUT_NAME}" \
  "${PYTHON}" benchmarks/nonlinear_mpc/benchmark_turbompc_autodiff_humanoid_mpc.py \
    --batch-sizes "${TURBOMPC_AUTODIFF_HUMANOID_BATCH_SIZES}" \
    --dtypes "${DTYPES}" \
    --sim-steps 20 \
    --max-iter "${MAX_ITER}" \
