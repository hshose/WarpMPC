#!/usr/bin/env python3
"""Batch-size sweep for the TurboMPC cartpole quadratic MPC baseline."""
...
EVALUATOR = ROOT / "benchmarks" / "nonlinear_mpc" / "evaluate_turbompc_cartpole_quadratic_mpc.py"
...
def run_batch(args: argparse.Namespace, *, batch_size: int, dtype: str) -> dict[str, object]:
    prefix = f"cartpole_quadratic_mpc_turbompc_{dtype}_batch_{batch_size}"
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=pathlib.Path, default=pathlib.Path("results/nonlinear_mpc/cartpole_quadratic_mpc_turbompc"))
    parser.add_argument("--csv-path", type=pathlib.Path, default=pathlib.Path("results/nonlinear_mpc/cartpole_quadratic_mpc_turbompc.csv"))
    parser.add_argument("--plot-path", type=pathlib.Path, default=pathlib.Path("results/nonlinear_mpc/cartpole_quadratic_mpc_turbompc_summary.png"))
