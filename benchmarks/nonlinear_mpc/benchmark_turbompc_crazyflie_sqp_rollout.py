#!/usr/bin/env python3
"""Batch-size sweep for the TurboMPC Crazyflie SQP-RTI baseline."""
...
EVALUATOR = ROOT / "benchmarks" / "nonlinear_mpc" / "evaluate_turbompc_crazyflie_sqp.py"
...
def run_batch(
    ...
    *,
    ...
    prefix = f"crazyflie_sqp_rollout_turbompc_{dtype}_batch_{batch_size}_run_{repeat_index + 1}"
    ...
    print(
        f"Running repeat={repeat_index + 1} solver=turbompc dtype={dtype} "
    ...
    if proc.returncode == 0 and summary_json.exists():
        ...
        row["error"] = ""
        print(
            f"repeat={repeat_index + 1} solver=turbompc dtype={dtype} batch={batch_size}: "
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=pathlib.Path, default=pathlib.Path("results/nonlinear_mpc/crazyflie_sqp_rollout_turbompc"))
    parser.add_argument("--csv-path", type=pathlib.Path, default=pathlib.Path("results/nonlinear_mpc/crazyflie_sqp_rollout_turbompc.csv"))
    parser.add_argument("--plot-path", type=pathlib.Path, default=pathlib.Path("results/nonlinear_mpc/crazyflie_sqp_rollout_turbompc_summary.png"))
