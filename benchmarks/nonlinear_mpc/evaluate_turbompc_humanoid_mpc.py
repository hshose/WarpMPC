#!/usr/bin/env python3
"""Closed-loop humanoid sparse MPC benchmark using TurboMPC dense time blocks."""
...
def run(args: argparse.Namespace) -> dict[str, object]:
    ...
    print("building humanoid TurboMPC dense-stage problem...", flush=True)
    problem_start = time.perf_counter()
    sparse_problem = make_humanoid_sqp_problem(
        args.horizon_nodes,
    ...
    problem = DenseStageTurbompcProblem(
        ...
        name="HumanoidDenseStageTurboMPC",
    )
...
def main() -> None:
    ...
    parser.add_argument("--plot-path", type=pathlib.Path, default=pathlib.Path("results/nonlinear_mpc/turbompc_humanoid_mpc_rollout.png"))
