#!/usr/bin/env python3
"""Closed-loop tuned cartpole MPC benchmark using TurboMPC."""
...
def run(args: argparse.Namespace) -> dict[str, object]:
    ...
    print("building cartpole TurboMPC dense-stage problem...", flush=True)
    problem_start = time.perf_counter()
    sparse_problem = make_cartpole_sqp_problem(
        args.horizon_steps,
    ...
    problem = DenseStageTurbompcProblem(
        ...
        name="CartpoleDenseStageTurboMPC",
    )
...
def main() -> None:
    ...
    parser.add_argument("--plot-path", type=pathlib.Path, default=pathlib.Path("results/nonlinear_mpc/turbompc_cartpole_rollout.png"))
