...
CARTPOLE_OUTPUT_NAME="${CARTPOLE_OUTPUT_NAME:-cartpole_quadratic_mpc_turbompc_autodiff}"
...
TURBOMPC_AUTODIFF_CARTPOLE_BATCH_SIZES="${TURBOMPC_AUTODIFF_CARTPOLE_BATCH_SIZES:-${TURBOMPC_CARTPOLE_BATCH_SIZES:-${CARTPOLE_BATCH_SIZES}}}"
...
run_benchmark "${CARTPOLE_OUTPUT_NAME}" \
  "${PYTHON}" benchmarks/nonlinear_mpc/benchmark_turbompc_autodiff_cartpole_quadratic_mpc.py \
    --batch-sizes "${TURBOMPC_AUTODIFF_CARTPOLE_BATCH_SIZES}" \
    --dtypes "${DTYPES}" \
    --horizon-steps 100 \
    --dt-start 0.1 \
    --dt-growth 1.0 \
