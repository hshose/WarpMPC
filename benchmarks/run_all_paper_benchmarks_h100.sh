...
RESULTS_ROOT="${1:-${ROOT}/results/h100_paper_benchmarks_${STAMP}}"
JOB_IDS_CSV="${RESULTS_ROOT}/job_ids.csv"
SBATCH_ACCOUNT_ARGS=()
if [[ -n "${SBATCH_ACCOUNT:-}" ]]; then
  SBATCH_ACCOUNT_ARGS=(--account="${SBATCH_ACCOUNT}")
fi
...
printf 'study,name,job_id,slurm_log,script\n' > "${JOB_IDS_CSV}"
...
submit_job() {
  ...
  shift 3
  local env_args=("$@")
  local sbatch_id job_id

  if sbatch_id="$(env "ROOT=${ROOT}" "${env_args[@]}" sbatch "${SBATCH_ACCOUNT_ARGS[@]}" --export=ALL --parsable "${script}" "${RESULTS_ROOT}")"; then
    job_id="${sbatch_id%%;*}"
    printf '%s,%s,%s,%s,%s\n' \
      "${study}" "${name}" "${job_id}" "${ROOT}/hpclogs/${job_id}.log" "${script}" \
      >> "${JOB_IDS_CSV}"
    echo "${study}/${name}: ${job_id}"
  else
    echo "${study}/${name}: sbatch failed"
  fi
}

echo "H100 paper benchmark submitter"
echo "Results:    ${RESULTS_ROOT}"
echo "Job IDs:    ${JOB_IDS_CSV}"
echo "Slurm logs: ${ROOT}/hpclogs/%A.log"
echo

submit_job "numpysadi_synthetic" "numpysadi_synthetic" "${ROOT}/benchmarks/numpysadi_synthetic/run_numpysadi_synthetic.sh"

submit_job "qdldl_factorization" "qdldl_factorization" "${ROOT}/benchmarks/qdldl_factorization/run_qdldl_factorization.sh" \
  "QDLDL_VARIANTS=${PAPER_QDLDL_VARIANTS}"
submit_job "qdldl_factorization" "cudss_factorization" "${ROOT}/benchmarks/qdldl_factorization/run_cudss_factorization.sh"
submit_job "qdldl_factorization" "dense_lu_factorization" "${ROOT}/benchmarks/qdldl_factorization/run_dense_lu_factorization.sh"
submit_job "qdldl_factorization" "jaxopt_cg_factorization" "${ROOT}/benchmarks/qdldl_factorization/run_jaxopt_cg_factorization.sh"

submit_job "linear_mpc" "jax_osqp_linear_mpc" "${ROOT}/benchmarks/linear_mpc/run_jax_osqp_linear_mpc.sh" \
  "VARIANTS=${PAPER_OSQP_VARIANTS}"
submit_job "linear_mpc" "boxosqp_linear_mpc" "${ROOT}/benchmarks/linear_mpc/run_boxosqp_linear_mpc.sh"
submit_job "linear_mpc" "qpax_linear_mpc" "${ROOT}/benchmarks/linear_mpc/run_qpax_linear_mpc.sh"
submit_job "linear_mpc" "mpax_linear_mpc" "${ROOT}/benchmarks/linear_mpc/run_mpax_linear_mpc.sh"

submit_job "linear_mpc_gradients" "jax_osqp_linear_mpc_gradients" "${ROOT}/benchmarks/linear_mpc_gradients/run_jax_osqp_linear_mpc_gradients.sh" \
  "VARIANTS=${PAPER_OSQP_VARIANTS}"
submit_job "linear_mpc_gradients" "boxosqp_linear_mpc_gradients" "${ROOT}/benchmarks/linear_mpc_gradients/run_boxosqp_linear_mpc_gradients.sh"
submit_job "linear_mpc_gradients" "qpax_linear_mpc_gradients" "${ROOT}/benchmarks/linear_mpc_gradients/run_qpax_linear_mpc_gradients.sh"
submit_job "linear_mpc_gradients" "mpax_linear_mpc_gradients" "${ROOT}/benchmarks/linear_mpc_gradients/run_mpax_linear_mpc_gradients.sh"

submit_job "nonlinear_mpc" "cartpole_quadratic_mpc" "${ROOT}/benchmarks/nonlinear_mpc/run_cartpole_quadratic_mpc.sh" \
...
submit_job "nonlinear_mpc" "cartpole_quadratic_mpc_mpax" "${ROOT}/benchmarks/nonlinear_mpc/run_mpax_cartpole_quadratic_mpc.sh" \
  "CARTPOLE_BATCH_SIZES=${PAPER_NONLINEAR_BATCH_SIZES}" \
  "MPAX_CARTPOLE_BATCH_SIZES=${PAPER_NONLINEAR_BATCH_SIZES}"
submit_job "nonlinear_mpc" "cartpole_quadratic_mpc_mpx" "${ROOT}/benchmarks/nonlinear_mpc/run_mpx_cartpole_quadratic_mpc.sh" \
  "CARTPOLE_BATCH_SIZES=${PAPER_NONLINEAR_BATCH_SIZES}" \
  "MPX_CARTPOLE_BATCH_SIZES=${PAPER_NONLINEAR_BATCH_SIZES}"
submit_job "nonlinear_mpc" "cartpole_quadratic_mpc_turbompc" "${ROOT}/benchmarks/nonlinear_mpc/run_turbompc_cartpole_quadratic_mpc.sh" \
  "CARTPOLE_BATCH_SIZES=${PAPER_NONLINEAR_BATCH_SIZES}" \
  "TURBOMPC_CARTPOLE_BATCH_SIZES=${PAPER_NONLINEAR_BATCH_SIZES}"
submit_job "nonlinear_mpc" "cartpole_quadratic_mpc_turbompc_autodiff" "${ROOT}/benchmarks/nonlinear_mpc/run_turbompc_autodiff_cartpole_quadratic_mpc.sh" \
  "CARTPOLE_BATCH_SIZES=${PAPER_NONLINEAR_BATCH_SIZES}" \
  "TURBOMPC_AUTODIFF_CARTPOLE_BATCH_SIZES=${PAPER_NONLINEAR_BATCH_SIZES}"
submit_job "nonlinear_mpc" "crazyflie_sqp_rollout" "${ROOT}/benchmarks/nonlinear_mpc/run_crazyflie_sqp_rollout.sh" \
  "CRAZYFLIE_BATCH_SIZES=${PAPER_NONLINEAR_BATCH_SIZES}" \
  "NONLINEAR_QDLDL_VARIANTS=${PAPER_NONLINEAR_QDLDL_VARIANTS}" \
  "LEVELSOLVE_MODES=${PAPER_LEVELSOLVE_MODES}"
submit_job "nonlinear_mpc" "crazyflie_sqp_rollout_mpax" "${ROOT}/benchmarks/nonlinear_mpc/run_mpax_crazyflie_sqp_rollout.sh" \
  "CRAZYFLIE_BATCH_SIZES=${PAPER_NONLINEAR_BATCH_SIZES}" \
  "MPAX_CRAZYFLIE_BATCH_SIZES=${PAPER_NONLINEAR_BATCH_SIZES}"
submit_job "nonlinear_mpc" "crazyflie_sqp_rollout_mpx" "${ROOT}/benchmarks/nonlinear_mpc/run_mpx_crazyflie_sqp_rollout.sh" \
  "CRAZYFLIE_BATCH_SIZES=${PAPER_NONLINEAR_BATCH_SIZES}" \
  "MPX_CRAZYFLIE_BATCH_SIZES=${PAPER_NONLINEAR_BATCH_SIZES}"
submit_job "nonlinear_mpc" "crazyflie_sqp_rollout_turbompc" "${ROOT}/benchmarks/nonlinear_mpc/run_turbompc_crazyflie_sqp_rollout.sh" \
  "CRAZYFLIE_BATCH_SIZES=${PAPER_NONLINEAR_BATCH_SIZES}" \
  "TURBOMPC_CRAZYFLIE_BATCH_SIZES=${PAPER_NONLINEAR_BATCH_SIZES}"
submit_job "nonlinear_mpc" "crazyflie_sqp_rollout_turbompc_autodiff" "${ROOT}/benchmarks/nonlinear_mpc/run_turbompc_autodiff_crazyflie_sqp_rollout.sh" \
  "CRAZYFLIE_BATCH_SIZES=${PAPER_NONLINEAR_BATCH_SIZES}" \
  "TURBOMPC_AUTODIFF_CRAZYFLIE_BATCH_SIZES=${PAPER_NONLINEAR_BATCH_SIZES}"
submit_job "nonlinear_mpc" "humanoid_mpc" "${ROOT}/benchmarks/nonlinear_mpc/run_humanoid_mpc.sh" \
  "HUMANOID_BATCH_SIZES=${PAPER_NONLINEAR_BATCH_SIZES}" \
  "NONLINEAR_QDLDL_VARIANTS=${PAPER_NONLINEAR_QDLDL_VARIANTS}" \
  "LEVELSOLVE_MODES=${PAPER_LEVELSOLVE_MODES}"
submit_job "nonlinear_mpc" "humanoid_mpc_mpax" "${ROOT}/benchmarks/nonlinear_mpc/run_mpax_humanoid_mpc.sh" \
  "HUMANOID_BATCH_SIZES=${PAPER_NONLINEAR_BATCH_SIZES}" \
  "MPAX_HUMANOID_BATCH_SIZES=${PAPER_NONLINEAR_BATCH_SIZES}"
submit_job "nonlinear_mpc" "humanoid_mpc_mpx" "${ROOT}/benchmarks/nonlinear_mpc/run_mpx_humanoid_mpc.sh" \
  "HUMANOID_BATCH_SIZES=${PAPER_NONLINEAR_BATCH_SIZES}" \
  "MPX_HUMANOID_BATCH_SIZES=${PAPER_NONLINEAR_BATCH_SIZES}"
submit_job "nonlinear_mpc" "humanoid_mpc_turbompc" "${ROOT}/benchmarks/nonlinear_mpc/run_turbompc_humanoid_mpc.sh" \
  "HUMANOID_BATCH_SIZES=${PAPER_NONLINEAR_BATCH_SIZES}" \
  "TURBOMPC_HUMANOID_BATCH_SIZES=${PAPER_NONLINEAR_BATCH_SIZES}"
submit_job "nonlinear_mpc" "humanoid_mpc_turbompc_autodiff" "${ROOT}/benchmarks/nonlinear_mpc/run_turbompc_autodiff_humanoid_mpc.sh" \
  "HUMANOID_BATCH_SIZES=${PAPER_NONLINEAR_BATCH_SIZES}" \
  "TURBOMPC_AUTODIFF_HUMANOID_BATCH_SIZES=${PAPER_NONLINEAR_BATCH_SIZES}"

submit_job "constraint_sqp_iteration_sweep" "cartpole_constraint_sqp_sweep" "${ROOT}/benchmarks/nonlinear_mpc/run_constraint_sqp_iteration_sweep.sh" \
...
submit_job "constraint_sqp_iteration_sweep" "crazyflie_obstacle_constraint_sqp_sweep" "${ROOT}/benchmarks/nonlinear_mpc/run_constraint_sqp_iteration_sweep.sh" \
  ...
  "CONSTRAINT_SWEEP_BATCH_SIZE=${SWEEP_BATCH_SIZE}" \
  "CONSTRAINT_SWEEP_CRAZYFLIE_HORIZON_STEPS=${SWEEP_CRAZYFLIE_HORIZON_STEPS}"
submit_job "constraint_sqp_iteration_sweep" "crazyflie_obstacle_rollout_paths" "${ROOT}/benchmarks/nonlinear_mpc/run_crazyflie_obstacle_rollout_paths.sh" \
  "CONSTRAINT_SWEEP_CRAZYFLIE_HORIZON_STEPS=${SWEEP_CRAZYFLIE_HORIZON_STEPS}" \
...
echo
echo "Submitted H100 paper benchmark jobs."
echo "Results: ${RESULTS_ROOT}"
echo "Job IDs: ${JOB_IDS_CSV}"
