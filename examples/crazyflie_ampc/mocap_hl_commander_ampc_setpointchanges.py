...
def run_experiment(cf: Crazyflie, args: argparse.Namespace, logconf: LogConfig | None = None) -> None:
    ...
    setpoint_y = (start[0] + args.dx, start[1] + args.dy, start[2], start[3])
    print(f"segment xy_step: setpoint={setpoint_y}, duration={args.step_s:.2f}s")
    send_position_for(cf, setpoint_y, args.step_s, args.setpoint_rate_hz)
    ...
    setpoint_z_down = (start[0] + args.dx, start[1] + args.dy, start[2] - args.dz, start[3])
    print(f"segment z_down: setpoint={setpoint_z_down}, duration={args.step_s:.2f}s")
    send_position_for(cf, setpoint_z_down, args.step_s, args.setpoint_rate_hz)
    ...
    setpoint_z_up = (start[0] + args.dx, start[1] + args.dy, start[2] + args.dz, start[3])
    print(f"segment z_up: setpoint={setpoint_z_up}, duration={args.step_s:.2f}s")
    send_position_for(cf, setpoint_z_up, args.step_s, args.setpoint_rate_hz)
    ...
    setpoint_yaw = (start[0] + args.dx, start[1] + args.dy, start[2] + args.yaw_z_offset, start[3] + args.dyaw)
    print(f"segment yaw_step: setpoint={setpoint_yaw}, duration={args.step_s:.2f}s")
    send_position_for(cf, setpoint_yaw, args.step_s, args.setpoint_rate_hz)

    print(f"segment return: setpoint={start}, duration={args.return_s:.2f}s")
    send_position_for(cf, start, args.return_s, args.setpoint_rate_hz)
